...
def StartScan(Dry, manageraddr):
    ...
    for item in nlist:
        ...
        udp_banner = ""
        tcp_pcount = 0
        # Se realmente recebeu uma tupla de porta aberta
        if p_tcp_l is not "" and p_tcp_l is not None:
            # entao vamos tokenizar cada tupla separada por virgula (se tiver
            # mais de1 porta aberta por processo)
            for svctcp in p_tcp_l.split(','):
                try:
                    ...
                    # print "ip: " + ip + " porta: " + porta + " banner: " +
                    # fp_item
                    if Dry == True:
                        print "PROCESS TCP FINGERPRINT: " + str(porta) + ":" + str(fp_item)
                    tcp_banner = tcp_banner + porta + ":" + \
                        b64encode(fp_item) + ":"  # porta:banner em base64
                    if PingManager(manageraddr) == 1:
                        sent_count += 1
                        SendData(manageraddr, server, domain, GetLinuxDist(DIST_NAME), GetLinuxDist(
                            DIST_VER), p_pid, p_name, p_uid, p_gid, p_rpm, p_dpkg, pf_path, pf_dac, pf_uid, pf_gid, p_args, tcp_banner, udp_banner)
                    else:
        ...
        udp_pcount = 0
        # Se realmente recebeu uma tupla de porta aberta
        if p_udp_l is not "" and p_udp_l is not None:
            # entao vamos tokenizar cada tupla separada por virgula (se tiver
            # mais de1 porta aberta por processo)
            for svcudp in p_udp_l.split(','):
                try:
                    ...
                    # porta:banner em base64
                    udp_banner = porta + ":" + b64encode(fp_item)
                    if PingManager(manageraddr) == 1:
                        sent_count += 1
                        SendData(manageraddr, server, domain, GetLinuxDist(DIST_NAME), GetLinuxDist(
                            DIST_VER), p_pid, p_name, p_uid, p_gid, p_rpm, p_dpkg, pf_path, pf_dac, pf_uid, pf_gid, p_args, tcp_banner, udp_banner)
