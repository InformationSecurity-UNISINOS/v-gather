...
def MatchData():
    ...
    for caso in lista_casos:
        ...
        while qlen > 0:
            ...
            p_pkg_ratio = Similarity(
                pdict['pacote'].lower(), db_case['package_name'].lower())
            ...
            ###################################################################
            p_name_ratio = Similarity(
                pdict['p_name'].lower(), db_case['process_name'].lower())
            ...
            # PROCESS UID:
            ###################################################################
            p_uid_ratio = Similarity(
                int(pdict['p_uid']), int(db_case['process_uid']))
            ...
            # PROCESS GID:
            ###################################################################
            p_gid_ratio = Similarity(
                int(pdict['p_gid']), int(db_case['process_gid']))
            ...
            ###################################################################
            p_args_ratio = Similarity(
                pdict['p_args'].lower(), db_case['process_args'].lower())
            ...
            ###################################################################
            p_tcp_banner_ratio = Similarity(
                pdict['p_tcp_banner'].lower(), db_case['process_tcp_banner'].lower())
            ...
            ###################################################################
            p_udp_banner_ratio = Similarity(
                pdict['p_udp_banner'].lower(), db_case['process_udp_banner'].lower())
            p_udp_banner_weight = float(db_case['process_udp_banner_weight'])
            p_udp_banner_score = float(
                p_udp_banner_ratio) * float(p_udp_banner_weight)
            ...
            # PROCESS FILE PATH
            ###################################################################
            pf_path_ratio = Similarity(
                pdict['pf_path'].lower(), db_case['process_binary'].lower())
            ...
            ###################################################################
            distro_version_ratio = Similarity(
                float(pdict['distro_version']), float(db_case['so_version']))
            distro_version_weight = float(db_case['so_version_weight'])
            distro_version_score = float(
                distro_version_ratio) * float(distro_version_weight)
            ...
            # PROCESS DISTRO NAME
            ###################################################################
            distro_ratio = Similarity(
                str(pdict['distro']).lower(), str(db_so_name).lower())
