...
def DbConnect():
    try:
        ret = MySQLdb.connect(
            host=sqlhost, port=3306, user=sqluser, passwd=sqlpass, db=sqldb)
    except:
        ret = None
    return ret
...
def DbSimCases():
    ...
    while clen > 0:
        ...
        query = "INSERT INTO use_cases ( status, origem, case_id_related, \
										...
										candidate_final_score) VALUES ( '%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s' )" % (
            2, 2, str(pdict2['case_id_related']), str(so_id), 
            str(pdict2['distro_weight']), str(pdict2['distro_score']),
            str(pdict2['distro_version']), str(pdict2['distro_version_weight']), 
            str(pdict2['distro_version_score']),
            str(pdict2['p_name']), str(pdict2['p_name_weight']), 
            str(pdict2['p_name_score']), str(pdict2['p_uid']),
            str(pdict2['p_uid_weight']), str(pdict2['p_uid_score']), 
            str(pdict2['p_gid']), str(pdict2['p_gid_weight']),
            str(pdict2['p_gid_score']), str(pdict2['p_args']), 
            str(pdict2['p_args_weight']), str(pdict2['p_args_score']),
            str(pdict2['p_tcp_banner']), str(pdict2['p_tcp_banner_weight']), 
            str(pdict2['p_tcp_banner_score']),
            str(pdict2['p_udp_banner']), str(pdict2['p_udp_banner_weight']), 
            str(pdict2['p_udp_banner_score']),
            str(pdict2['p_package']), str(pdict2['p_pkg_weight']), 
            str(pdict2['p_pkg_score']), str(pdict2['pf_path']),
            str(pdict2['pf_path_weight']), str(pdict2['pf_path_score']), str(round(pdict2['score'], 2)))
